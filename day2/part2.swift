...
for (idx, game) in games.enumerated() {
    ...
    var maxColors = ["red": 0, "green": 0, "blue": 0]
    for `set` in game {
    ...
    print("Game \(idx + 1): MaxColors -> \(maxColors)")
    
    let (blueAmount, greenAmount, redAmount) = (maxColors["blue"]!, maxColors["green"]!, maxColors["red"]!)
    powerSum += (blueAmount * greenAmount * redAmount)
}
